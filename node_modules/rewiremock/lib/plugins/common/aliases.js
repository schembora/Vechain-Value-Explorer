...
function getConfigPath(configPaths) {
  ...
  (0, _lodash4.default)(configPaths, configPath => {
    ...
    let resolvedConfigPath = (0, _path.resolve)(process.cwd(), compiledConfigPath);
    const resolvedName = (0, _utils.fileExists)(resolvedConfigPath);
...
function readAliases(configPath) {
  ...
  // Require the config
  let conf = (0, _executor.requireModule)(confPath);
  ...
  if (typeof __webpack_modules__ !== 'undefined') Object.keys(aliasConf).forEach(alias => {
  ...
  for (const aliasFrom in aliasConf) {
    if (aliasConf.hasOwnProperty(aliasFrom)) {
      ...
      if (regex.test(filePath)) {
        ...
        let relativeFilePath = (0, _path.resolve)((0, _path.dirname)(filePath), aliasTo).split(_path.sep).join('/');
